...
|---|---|---|
| ~~Capture/cancel endpoints incorrectos~~ | ✅ RESUELTO — sesión julio 2026 | Verificado con la doc oficial de Paylands (docs.paylands.com/en/reference): capture real es `POST /payment/confirmation` (no `/payment/capture`) y cancel real es `POST /payment/cancellation` (no `/payment/cancel`). **Hallazgo más importante**: ambos endpoints SOLO operan sobre órdenes creadas con `operative: "DEFERRED"` — con `AUTHORIZATION` (lo que usábamos) el dinero se mueve al instante y no hay nada que confirmar/cancelar. Cambiado `operative` a `DEFERRED` en las tres funciones de creación de orden (`createOrder`, `createOrder3DS`, `chargeWithToken`) de `payNoPainConnector.js`. **Pendiente de verificar**: que el webhook de una autorización DEFERRED se siga mapeando a `authorized` en local igual que antes — probar con transacciones nuevas creadas DESPUÉS de este cambio (las autorizadas antes de este commit siguen siendo `AUTHORIZATION` y nunca podrán capturarse/cancelarse). |
...
*Última revisión: 12 julio 2026 (sesión tarde, cont.) — Capture/cancel: URLs reales confirmadas con doc oficial de Paylands (`/payment/confirmation` y `/payment/cancellation`, no `/capture`/`/cancel`). Hallazgo clave: ambas SOLO operan sobre órdenes `operative: DEFERRED` — cambiado desde `AUTHORIZATION` en `createOrder`/`createOrder3DS`/`chargeWithToken`. Fix adicional: refund rechazaba reembolsos parciales porque solo aceptaba el status `REFUNDED` exacto de Paylands, no `PARTIALLY_REFUNDED` (bug real, confirmado con log de producción — ya corregido, igual que el mapeo en el webhook). Fix de validación: `refundSchema` exigía `amountOfMoney` obligatorio a diferencia de capture/cancel, ahora opcional igual que los otros dos. Refund YA VERIFICADO funcionando end-to-end contra Paylands real (total y parcial). Capture/cancel pendientes de reverificar con transacciones nuevas (las anteriores a este cambio quedaron creadas como AUTHORIZATION y no se pueden capturar/cancelar). M3 CERRADO: nueva pestaña Reglas absorbe el editor viejo (index.html/app.js) sobre sesión JWT de backoffice. Pestañas Merchants y API Keys añadidas. Widgets del dashboard expandibles al tocarlos. M1, M2 y M3 completados. Tarjetas test buenas: 4018810...*
